...
/* ==============================
   Demo Data — replace with your real videos
   ============================== */
const VIDEOS: VideoItem[] = [
  ...
  },
  {
    ...
    title: "Theme Parks & EntertainmentConfex  - 2023",
    provider: "youtube",
    src: "qsz7OoIcBho",
    duration: "00:51",
    year: 2023,
  ...
  },
  {
    ...
    year: 2022,
    country: "Oman",
    tags: ["Asset Integrity", "Integrity", "Management", "Summit", "AIM","Oman"],
  },
  {
  ...
  },
  {
    ...
    title: "Digital Transformation Public Sector Executive Boardroom Meeting Highlights - 2022",
    provider: "youtube",    
    src: "SPHg6DgLCSk",
    duration: "00:41",    
    year: 2022,
    country: "UAE",
    tags: ["Digital Transformation", "Public Sector", "Executive Boardroom Meeting", "UAE", "Boardroom"],
  },
  {
  ...
  return (
    <AnimatePresence>
      {open && video && (
        ...
        >
          <motion.div
            className="relative w-full max-w-5xl aspect-video  overflow-hidden bg-black"
          ...
          >
            ...
            <div className="pointer-events-none absolute inset-x-0 bottom-0 bg-gradient-to-t from-black/60 to-transparent p-4">
              <h3 className="text-white text-sm sm:text-base font-semibold">{video.title}</h3>
            </div>
          </motion.div>
        </motion.div>
      )}
...
function useTilt(max = 6) {
  ...
  const onMove = (e: React.MouseEvent) => {
    ...
    el.style.transform = `perspective(1000px) rotateX(${(py - 0.5) * -max}deg) rotateY(${(px - 0.5) * max}deg) scale(1.01)`;
...
/* ==============================
   Video Card — modern cinematic (gapless + tilt + YT thumbs)
   ============================== */
function VideoCard({
  v,
  onOpen,
  ...
  // Always use YouTube thumbnail for youtube items
  const thumb =
    v.provider === "youtube" ? ytThumb(v.src) : (v.thumbnail ?? "/images/fallback-video.jpg");
  ...
  return (
    ...
    >
      ...
      >
        ...
        {/* dark veil + soft glare that follows cursor */}
        <div className="absolute inset-0 bg-black/28 transition group-hover:bg-black/40" />
        <div className="pointer-events-none absolute inset-0 rounded-[inherit] opacity-0 transition group-hover:opacity-100"
             style={{
               background:
        ...
        {/* bottom-left title/subtitle (uppercase, tracking) */}
...
/* ==============================
   Page (unchanged except gapless grid + dark stage behind tiles)
   ============================== */
export default function EventsHighlightVideoGallery() {
    ...
    <div className="min-h-screen bg-white">
      ...
      <header className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-8">
        <motion.div
          ...
          className="rounded-3xl bg-darkblue-900 text-white p-6 sm:p-8 shadow-lg"
        >
          <div className="flex flex-col gap-6 md:flex-row md:items-center md:justify-between">
            <div>
              ...
              <h1 className="mt-3 text-3xl sm:text-4xl font-semibold">Here is what we've done.</h1>
              <p className="mt-2 max-w-2xl text-sm opacity-90">
                Watch quick recaps and highlight reels from our flagship conferences and summits.
              </p>
            </div>
            ...
            <div className="grid grid-cols-1 gap-2 sm:grid-cols-3 md:w-[520px]">
              <label className="relative col-span-1 sm:col-span-3">
                <Search className="pointer-events-none absolute left-3 top-1/2 -translate-y-1/2 h-4.5 w-4.5 text-slate-400" />
                <input
                  value={q}
                  onChange={(e) => setQ(e.target.value)}
                  placeholder="Search title, city, tag…"
                  className="w-full rounded-xl border border-darkblue-300 bg-white pl-10 pr-3 py-2.5 text-slate-800 outline-none focus:ring-4 ring-darkblue-300"
                />
              </label>
      ...
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 pb-2">
        <div className="flex items-center justify-between text-sm text-neutral-600">
          <span>
            Showing <b>{results.length}</b> {results.length === 1 ? "video" : "videos"}
          </span>
          {(q || year !== "All" || tag !== "All") && (
            <button
              className="text-darkblue-700 underline decoration-darkblue-300 underline-offset-4 hover:opacity-80"
              onClick={() => {
                setQ("");
                setYear("All");
                setTag("All");
      ...
      {/* Grid — gapless + dark stage behind tiles for the cinematic feel */}
      <main className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 pb-16">
        {results.length === 0 ? (
          <div className="grid place-items-center  border border-dashed border-neutral-300 p-10 text-neutral-600">
        ...
        {/* <p className="mt-6 text-xs text-neutral-500">
          Note: YouTube embeds use <code>modestbranding=1</code> and <code>controls=0</code>. Full removal of branding isn’t supported YouTube policies.
        </p> */}
      </main>
      ...
      <AnimatePresence>
        {open && (
          <motion.ul
            ...
            className="absolute z-[40] mt-2 w-full overflow-hidden rounded-xl border border-neutral-200 bg-white shadow-lg"
            role="listbox"
          >
            {options.map((opt) => (
              <li key={opt}>
                <button
                  className={cx(
                    "w-full px-3 py-2 text-sm text-left hover:bg-darkblue-50",
                    value === opt ? "bg-darkblue-50 text-darkblue-800" : "text-slate-800"
                  )}
                  role="option"
