...
export default function ExploreSwitch({ active, onChange, counts }: Props) {
  return (
    <div className="flex justify-center px-4 py-6 sm:py-10">
      <div className="flex items-center gap-1.5 sm:gap-2 bg-white px-2 sm:px-3 py-2 rounded-full shadow-sm border border-gray-200">
        {tabs.map((tab) => (
          <button
            key={tab.key}
            onClick={() => onChange(tab.key as ExploreCategory)}
            className={clsx(
              "relative flex items-center justify-center gap-1.5 sm:gap-2 rounded-full px-4 sm:px-5 py-2.5 sm:py-2 text-sm font-semibold transition-all duration-300 whitespace-nowrap",
              active === tab.key
          ...
          >
            ...
            {counts?.[tab.key as ExploreCategory] && (
              <span
                className={clsx(
                  "ml-0.5 sm:ml-1 flex h-5 min-w-[20px] items-center justify-center rounded-full text-xs font-bold transition-all",
                  active === tab.key
